...
class ApiAccess():
    ...
    def RunCmd(self, varsIn: dict = None):
        try:
            ...
            match model:
                case "PetPost":
                    import components.petpost.petpost as petpost
                    petpost_instance = petpost.petpost()

                    varsIn1 = {'name': varsIn.get("name"),
                               'age': varsIn.get("age"),
                               'breed': varsIn.get("breed"),
                               'image': varsIn.get("image")}
                    return petpost_instance.Gsad(op, varsIn1)
                case _:
